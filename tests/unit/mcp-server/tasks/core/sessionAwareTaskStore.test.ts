...
describe('SessionAwareTaskStore', () => {
  ...
  it('removes ownership after a terminal result is stored', async () => {
    ...
    await expect(store.getTaskResult(task.taskId, 'session-b')).resolves.toEqual(testResult);
  ...
  it('keeps ownership for non-terminal status updates and clears it for terminal ones', async () => {
    ...
    await expect(store.getTask(task.taskId, 'session-b')).resolves.toEqual(task);
    expect(innerUpdateTaskStatus).toHaveBeenNthCalledWith(
      1,
      task.taskId,
      'working',
      'still processing',
