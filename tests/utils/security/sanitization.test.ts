...
describe('Sanitization Utility', () => {
  describe('sanitizeHtml', () => {
    it('should remove <script> tags and other malicious HTML', () => {
      const maliciousInput = '<script>alert("xss")</script><p>Hello</p><iframe src="http://example.com"></iframe>';
  ...
  describe('sanitizeString', () => {
    ...
    it('should throw an McpError when context is "javascript"', () => {
      ...
      expect(() => sanitization.sanitizeString(jsInput, { context: 'javascript' })).toThrow(McpError);
  ...
  describe('sanitizePath', () => {
    ...
    it('should throw an error for paths containing null bytes (\\0)', () => {
      ...
      expect(() => sanitization.sanitizePath(nullBytePath)).toThrow(
        expect.objectContaining({ code: BaseErrorCode.VALIDATION_ERROR, message: 'Path contains null byte, which is disallowed.' })
      );
    });
    ...
    it('should handle absolute paths correctly based on the allowAbsolute option', () => {
        ...
        expect(() => sanitization.sanitizePath(absolutePath, { allowAbsolute: false })).toThrow(McpError);
        // When allowed, the path should be returned as is.
        expect(sanitization.sanitizePath(absolutePath, { allowAbsolute: true }).sanitizedPath).toBe(absolutePath);
    });
  });
  ...
  describe('sanitizeForLogging', () => {
    ...
    it('should redact sensitive keys in a deeply nested object', () => {
      ...
      const sanitized = sanitization.sanitizeForLogging(sensitiveObject) as Record<string, any>;
    ...
    it('should handle arrays of objects correctly', () => {
        ...
        const sanitized = sanitization.sanitizeForLogging(sensitiveArray) as Record<string, unknown>[];
