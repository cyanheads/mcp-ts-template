...
describe("SchedulerService", () => {
  ...
  describe("schedule", () => {
    it("should successfully schedule a new job with valid cron pattern", () => {
      ...
      const job = scheduler.schedule(jobId, schedule, mockTaskFunction, description);
    ...
    it("should throw error when providing invalid cron schedule", () => {
      ...
      expect(() => {
        scheduler.schedule(jobId, invalidSchedule, mockTaskFunction, description);
    ...
    it("should validate different cron patterns correctly", () => {
      ...
      validSchedules.forEach((schedule, index) => {
        ...
        expect(() => {
          scheduler.schedule(jobId, schedule, mockTaskFunction, `Valid job ${index}`);
        }).not.toThrow();
      });
    });
  });
  ...
  describe("start", () => {
    it("should start an existing job successfully", () => {
      ...
      const job = scheduler.schedule(jobId, "0 0 * * *", mockTaskFunction, "Startable job");
  ...
  describe("stop", () => {
    it("should stop an existing job successfully", () => {
      ...
      const job = scheduler.schedule(jobId, "0 0 * * *", mockTaskFunction, "Stoppable job");
  ...
  describe("remove", () => {
    it("should remove an existing job successfully", () => {
      ...
      const job = scheduler.schedule(jobId, "0 0 * * *", mockTaskFunction, "Removable job");
  ...
  describe("listJobs", () => {
    ...
    it("should return all scheduled jobs", () => {
      ...
      const job1 = scheduler.schedule(job1Id, "0 0 * * *", mockTaskFunction, "First job");
      const job2 = scheduler.schedule(job2Id, "0 12 * * *", mockTaskFunction, "Second job");
    ...
    it("should reflect job state changes in listed jobs", () => {
      ...
      scheduler.schedule(jobId, "0 0 * * *", mockTaskFunction, "State tracking job");
  ...
  describe("Integration with node-cron", () => {
    ...
    it("should create actual ScheduledTask objects", () => {
      const job = scheduler.schedule("task-test", "0 0 * * *", mockTaskFunction, "Task test");
